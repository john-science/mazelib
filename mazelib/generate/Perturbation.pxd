...
cdef class Perturbation(MazeGenAlgo):
    ...
    @cython.locals(passages=list, found=bint, r=cython.int, c=cython.int, ns=list, current=set, i=cython.int,
                   passage=list, intersect=set)
    cdef inline ndarray[cython.char, ndim=2] _find_all_passages(self, ndarray[cython.char, ndim=2] grid)

    @cython.locals(found=bint, cell=tuple, neighbors=list, passage=list, intersect=list, c=tuple, cell=tuple,
                   mid=tuple)
