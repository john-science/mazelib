...
* *new_walls*: Integer [1, ...)
 * The number of randomly positioned new walls you create throughout the maze. (default 3)
* *repeat*: Integer [1, ...)
 * The number of times sets of new walls will be added to the maze; the maze being fixed after each set.
...
In math and science, perturbation theory is the idea of apply small changes to the mathematical solutions to complex problems. Here, we take a solved maze and perturb it slightly by adding a couple walls, then re-solving the maze. This is an amazingly powerful tool. It can fix nearly any flaw in a maze. Or you can start with a non-maze, say a nice spiral or a cute drawing, and turn it into a maze using first-order perturbations.
